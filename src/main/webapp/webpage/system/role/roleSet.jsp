...
<script type="text/javascript">
	...
	function mysubmit() {
		var roleId = $("#rid").val();
		var s = GetNode();
		doSubmitAlertTip("roleController.do?updateAuthority&rolefunctions=" + s + "&roleId=" + roleId);
	}
	function GetNode() {
...
//	}).addClass("button").removeClass("linkbutton"); 
//	$('#resetBtn').linkbutton({   
//	});   
</script>
...
<div class="easyui-panel" style="padding: 1px;" fit="true" border="false" id="functionListPanel"><input type="hidden" name="roleId" value="${roleId}" id="rid">
 <a id="selecrAllBtn" class="button"
	onclick="selecrAll();"><t:mutiLang langKey="select.all"/></a>
	 <a id="resetBtn" class="button" onclick="reset();"><t:mutiLang langKey="common.reset"/></a>
<ul id="functionid"></ul>
</div>
</div>
