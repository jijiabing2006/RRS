...
<div class="easyui-layout" fit="true">
  ...
  <t:datagrid name="jshFList" checkbox="true" fitColumns="true" title="购汇管理信息" actionUrl="jshFController.do?datagrid" 
 idField="id" fit="true" queryMode="group" pageSize="30" sortName="importdate" sortOrder="desc" autoLoadData="true" >
   ...
   <t:dgCol title="结汇用途"  field="usetype"  hidden="true"  queryMode="group" dictionary="s_p_jshpurpose,code,text" ></t:dgCol>
  ...
  <t:dgCol title="r.importdate"  field="importdate" hidden="false" formatter="yyyy-MM-dd" query="true" queryMode="group" ></t:dgCol>
   ...
   <t:dgDelOpt operationCode="del" title="common.delete" exp="isvalidation#eq#0" url="jshFController.do?doDel&id={id}" />
   <t:dgToolBar operationCode="add" title="common.add" icon="icon-add" url="jshFController.do?goAdd" funname="add"></t:dgToolBar>
   <t:dgToolBar operationCode="edit" title="common.edit" icon="icon-edit" url="jshFController.do?goUpdate" width="810"  funname="update"></t:dgToolBar>
   <t:dgToolBar operationCode="detail" title="common.view"  icon="icon-search" url="jshFController.do?goUpdate" funname="detail"></t:dgToolBar>
   ...
   <t:dgToolBar  operationCode="batchApprove" title="r.batch.approve" icon="icon-ok" url="jshController.do?doBatchApprove&type=F" funname="approveALLSelect"></t:dgToolBar>
    <t:dgToolBar  operationCode="batchDel" title="common.batch.delete"  icon="icon-remove"  url="jshFController.do?doBatchDel" funname="deleteALLSelect"></t:dgToolBar>

  </t:datagrid>
  </div>
 </div>
 <script src = "webpage/safe/jsh/jshFList.js"></script>		
 <script type="text/javascript">
 $(document).ready(function(){
 		//给时间控件加上样式
 			$("#jshFListtb").find("input[name='rptdate_begin']").attr("class","Wdate").click(function(){WdatePicker({dateFmt:'yyyy-MM-dd'});});
 			$("#jshFListtb").find("input[name='rptdate_end']").attr("class","Wdate").click(function(){WdatePicker({dateFmt:'yyyy-MM-dd'});});
 			$("#jshFListtb").find("input[name='importdate_begin']").attr("class","Wdate").click(function(){WdatePicker({dateFmt:'yyyy-MM-dd'});});
 			$("#jshFListtb").find("input[name='importdate_end']").attr("class","Wdate").click(function(){WdatePicker({dateFmt:'yyyy-MM-dd'});});
 });
 
//导入
function ImportXls() {
	openuploadwin('Excel导入', 'jshFController.do?upload', "jshFList");
}

//导出
function ExportXls() {
	JeecgExcelExport("jshFController.do?exportXls","jshFList");
}

//模板下载
function ExportXlsByT() {
	JeecgExcelExport("jshFController.do?exportXlsByT","jshFList");
}
function approveCancel(id) {
	createwindow('审核重置', 'jshController.do?doCancelAuthor&id=' + id+'&type=F');
}
