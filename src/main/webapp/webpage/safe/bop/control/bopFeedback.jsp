...
<div class="easyui-layout" fit="true">
	<div region="center" style="padding:1px;">
		<t:datagrid name="fList" title="读取反馈"
			actionUrl="bopController.do?findFeedback" idField="id" fit="true"
			...
			<t:dgCol title="common.operation" field="opt"></t:dgCol>
			   <t:dgFunOpt  title="common.view"  funname="viewbop(fbh_filename)" operationCode="view" />
...
<script type="text/javascript">
	$(function() {
		...
		$("#fListtb").find("input[name='fbh.importdate']").attr("class", "Wdate")
				.attr("style", "height:20px;width:120px;").click(function() {
					WdatePicker({
						dateFmt : 'yyyy-MM-dd'
					});
				});
	...
	function readFeedback() {
		...
		doSubmitNunTable("bopController.do?readFeedback&date=" + d, "formobj");
	}
	function viewbop(filename) {
	   var s=filename.substr(0,3);
	   var subtitle="国际收支信息";
	   if(s=="BOP"){
		   subtitle="国际收支信息";
	   }
//	addOneTab(subtitle, "accController.do?goaccupdate&type="+s+"&");
 //   update(subtitle,"accController.do?goaccupdate&load=detail","fList");
    detail(subtitle,"bopController.do?gobopupdate","fList");
	}
</script>