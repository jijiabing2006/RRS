...
<div class="easyui-layout" fit="true">
  ...
  <t:datagrid name="bopEList" checkbox="true" fitColumns="true"  title="境内汇款申请书基础信息" actionUrl="bopEController.do?datagrid"
   ...
   <t:dgDelOpt operationCode="del" title="common.delete" exp="isvalidation#eq#0" url="bopEController.do?doDel&id={id}" />
   <t:dgToolBar operationCode="add" title="common.add" icon="icon-add" url="bopEController.do?goAdd" width="1036"  funname="add"></t:dgToolBar>
   <t:dgToolBar operationCode="edit" title="common.edit" icon="icon-edit" url="bopEController.do?goUpdate" width="1036"  funname="update"></t:dgToolBar>
   <t:dgToolBar operationCode="detail" title="common.view"  icon="icon-search" url="bopEController.do?goUpdate" width="1036"  funname="detail"></t:dgToolBar>
   ...
   <t:dgToolBar  operationCode="batchApprove" title="r.batch.approve" icon="icon-ok" url="bopController.do?doBatchApprove&type=E" funname="approveALLSelect"></t:dgToolBar>
    <t:dgToolBar  operationCode="batchDel" title="common.batch.delete"  icon="icon-remove"  url="bopEController.do?doBatchDel" funname="deleteALLSelect"></t:dgToolBar>

  </t:datagrid>
  </div>
 </div>
 <script src = "webpage/safe/bop//bopEList.js"></script>		
 <script type="text/javascript">
 $(document).ready(function(){
 		//给时间控件加上样式
 			$("#bopEListtb").find("input[name='importdate_begin']").attr("class","Wdate").click(function(){WdatePicker({dateFmt:'yyyy-MM-dd'});});
 			$("#bopEListtb").find("input[name='importdate_end']").attr("class","Wdate").click(function(){WdatePicker({dateFmt:'yyyy-MM-dd'});});
 });
 
//导入
function ImportXls() {
	openuploadwin('Excel导入', 'bopEController.do?upload', "bopEList");
}

//导出
function ExportXls() {
	JeecgExcelExport("bopEController.do?exportXls","bopEList");
}

//模板下载
function ExportXlsByT() {
	JeecgExcelExport("bopEController.do?exportXlsByT","bopEList");
}
function approveCancel(id) {
	createwindow('审核重置', 'bopController.do?doCancelAuthor&id=' + id+'&type=E');
}
function goControldata(id) {
	createwindow('管理信息', 'bopController.do?goControldata&id=' + id+'&type=E',1036,280);
}
 </script>