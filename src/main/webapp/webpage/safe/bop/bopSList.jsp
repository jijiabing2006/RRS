...
<div class="easyui-layout" fit="true">
  ...
  <t:datagrid name="bopSList" checkbox="true" fitColumns="true" title="境内付款/承兑通知书-管理信息" actionUrl="bopSController.do?datagrid" 
   ...
   <t:dgCol title="付款类型"  field="paytype"    queryMode="group" dictionary="paytype" ></t:dgCol>
   ...
   <t:dgDelOpt operationCode="del" title="common.delete" exp="isvalidation#eq#0" url="bopSController.do?doDel&id={id}" />
   <t:dgToolBar operationCode="add" title="common.add" icon="icon-add" url="bopSController.do?goAdd" funname="add"></t:dgToolBar>
   <t:dgToolBar operationCode="edit" title="common.edit" icon="icon-edit" url="bopSController.do?goUpdate" width="1036"  funname="update"></t:dgToolBar>
   <t:dgToolBar operationCode="detail" title="common.view"  icon="icon-search" url="bopSController.do?goUpdate" funname="detail"></t:dgToolBar>
   ...
   <t:dgToolBar  operationCode="batchApprove" title="r.batch.approve" icon="icon-ok" url="bopController.do?doBatchApprove&type=S" funname="approveALLSelect"></t:dgToolBar>
    <t:dgToolBar  operationCode="batchDel" title="common.batch.delete"  icon="icon-remove"  url="bopSController.do?doBatchDel" funname="deleteALLSelect"></t:dgToolBar>
  </t:datagrid>
  </div>
 </div>
 <script src = "webpage/safe/bop/bopSList.js"></script>		
 <script type="text/javascript">
 $(document).ready(function(){
 		//给时间控件加上样式
 			$("#bopSListtb").find("input[name='rptdate_begin']").attr("class","Wdate").click(function(){WdatePicker({dateFmt:'yyyy-MM-dd'});});
 			$("#bopSListtb").find("input[name='rptdate_end']").attr("class","Wdate").click(function(){WdatePicker({dateFmt:'yyyy-MM-dd'});});
 			$("#bopSListtb").find("input[name='importdate_begin']").attr("class","Wdate").click(function(){WdatePicker({dateFmt:'yyyy-MM-dd'});});
 			$("#bopSListtb").find("input[name='importdate_end']").attr("class","Wdate").click(function(){WdatePicker({dateFmt:'yyyy-MM-dd'});});
 });
 
//导入
function ImportXls() {
	openuploadwin('Excel导入', 'bopSController.do?upload', "bopSList");
}

//导出
function ExportXls() {
	JeecgExcelExport("bopSController.do?exportXls","bopSList");
}

//模板下载
function ExportXlsByT() {
	JeecgExcelExport("bopSController.do?exportXlsByT","bopSList");
}function approveCancel(id) {
	createwindow('审核重置', 'bopController.do?doCancelAuthor&id=' + id+'&type=S');
}
 </script>