...
<div class="easyui-layout" fit="true">
  ...
  <t:datagrid name="bopCList" checkbox="true" fitColumns="true" title="对外付款承兑通知书基础信息" actionUrl="bopCController.do?datagrid"
  ...
  <t:dgCol title="r.currency.code"  field="txccy"   ></t:dgCol>
   ...
   <t:dgDelOpt operationCode="del" title="common.delete" exp="isvalidation#eq#0" url="bopCController.do?doDel&id={id}" />
   <t:dgToolBar operationCode="add" title="common.add" icon="icon-add" url="bopCController.do?goAdd" funname="add"></t:dgToolBar>
   <t:dgToolBar operationCode="edit" title="common.edit" icon="icon-edit" url="bopCController.do?goUpdate" width="1036" height="500"  funname="update"></t:dgToolBar>
   <t:dgToolBar operationCode="detail" title="common.view"  icon="icon-search" url="bopCController.do?goUpdate" funname="detail"></t:dgToolBar>
   ...
   <t:dgToolBar  operationCode="batchApprove" title="r.batch.approve" icon="icon-ok" url="bopController.do?doBatchApprove&type=C" funname="approveALLSelect"></t:dgToolBar>
    <t:dgToolBar  operationCode="batchDel" title="common.batch.delete"  icon="icon-remove"  url="bopCController.do?doBatchDel" funname="deleteALLSelect"></t:dgToolBar>
  </t:datagrid>
  </div>
 </div>
 <script src = "webpage/safe/bop/bopCList.js"></script>		
 <script type="text/javascript">
 $(document).ready(function(){
 		//给时间控件加上样式
 			$("#bopCListtb").find("input[name='importdate_begin']").attr("class","Wdate").click(function(){WdatePicker({dateFmt:'yyyy-MM-dd'});});
 			$("#bopCListtb").find("input[name='importdate_end']").attr("class","Wdate").click(function(){WdatePicker({dateFmt:'yyyy-MM-dd'});});
 });
 
//导入
function ImportXls() {
	openuploadwin('Excel导入', 'bopCController.do?upload', "bopCList");
}

//导出
function ExportXls() {
	JeecgExcelExport("bopCController.do?exportXls","bopCList");
}

//模板下载
function ExportXlsByT() {
	JeecgExcelExport("bopCController.do?exportXlsByT","bopCList");
}
function approveCancel(id) {
	createwindow('审核重置', 'bopController.do?doCancelAuthor&id=' + id+'&type=C');
}
function goDeclaredata(id) {
	createwindow('申报信息', 'bopController.do?goDeclaredata&id=' + id+'&type=C',1036,300);
}
function goControldata(id) {  
	createwindow('管理信息', 'bopController.do?goControldata&id=' + id+'&type=C',1036,200);
}
 </script>