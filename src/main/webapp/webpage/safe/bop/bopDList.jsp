...
<div class="easyui-layout" fit="true">
  ...
  <t:datagrid name="bopDList" checkbox="true" fitColumns="true" title="境内收入申报单基础信息" actionUrl="bopDController.do?datagrid"
   ...
   <t:dgDelOpt operationCode="del" title="common.delete" exp="isvalidation#eq#0" url="bopDController.do?doDel&id={id}" />
   <t:dgToolBar operationCode="add" title="common.add" icon="icon-add" url="bopDController.do?goAdd" width="1036" funname="add"></t:dgToolBar>
   <t:dgToolBar operationCode="edit" title="common.edit" icon="icon-edit" url="bopDController.do?goUpdate" width="1036"  funname="update"></t:dgToolBar>
   <t:dgToolBar operationCode="detail" title="common.view"  icon="icon-search" url="bopDController.do?goUpdate" width="1036" funname="detail"></t:dgToolBar>
   ...
   <t:dgToolBar  operationCode="batchApprove" title="r.batch.approve" icon="icon-ok" url="bopController.do?doBatchApprove&type=D" funname="approveALLSelect"></t:dgToolBar>
    <t:dgToolBar  operationCode="batchDel" title="common.batch.delete"  icon="icon-remove"  url="bopDController.do?doBatchDel" funname="deleteALLSelect"></t:dgToolBar>
 
  </t:datagrid>
  </div>
 </div>
 <script src = "webpage/safe/bop/bopDList.js"></script>		
 <script type="text/javascript">
 $(document).ready(function(){
 		//给时间控件加上样式
 			$("#bopDListtb").find("input[name='importdate_begin']").attr("class","Wdate").click(function(){WdatePicker({dateFmt:'yyyy-MM-dd'});});
 			$("#bopDListtb").find("input[name='importdate_end']").attr("class","Wdate").click(function(){WdatePicker({dateFmt:'yyyy-MM-dd'});});
 });
 
//导入
function ImportXls() {
	openuploadwin('Excel导入', 'bopDController.do?upload', "bopDList");
}

//导出
function ExportXls() {
	JeecgExcelExport("bopDController.do?exportXls","bopDList");
}

//模板下载
function ExportXlsByT() {
	JeecgExcelExport("bopDController.do?exportXlsByT","bopDList");
}
function approveCancel(id) {
	createwindow('审核重置', 'bopController.do?doCancelAuthor&id=' + id+'&type=D');
}
function goControldata(id) {
	createwindow('管理信息', 'bopController.do?goControldata&id=' + id+'&type=D',1036,280);
}
 </script>