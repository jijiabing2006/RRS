...
<div class="easyui-layout" fit="true">
  ...
  <t:datagrid name="bopGList" checkbox="true" fitColumns="true" title="涉外收入-申报信息" actionUrl="bopGController.do?datagrid" 
   ...
   <t:dgCol title="修改删除原因"  field="actiondesc"  hidden="true"  queryMode="group"  ></t:dgCol>
   ...
   <t:dgCol title="付款人常驻国家地区代码"  field="country"    queryMode="group" dictionary="s_p_countrycode,code,text" ></t:dgCol>
   <t:dgCol title="收款性质"  field="paytype"    queryMode="group" dictionary="paytype" ></t:dgCol>
   ...
   <t:dgCol title="外汇局批件号"  field="billno"  hidden="true"  queryMode="group"  ></t:dgCol>
   <t:dgCol title="收入类型"  field="payattr"  hidden="true"  queryMode="group" dictionary="payattr" ></t:dgCol>
   ...
   <t:dgCol title="r.importdate"  field="importdate" hidden="false" formatter="yyyy-MM-dd" query="false" queryMode="group" ></t:dgCol>
   ...
   <t:dgDelOpt operationCode="del" title="common.delete" exp="isvalidation#eq#0" url="bopGController.do?doDel&id={id}" />
   <t:dgToolBar operationCode="add" title="common.add" icon="icon-add" url="bopGController.do?goAdd" width="1036"  funname="add"></t:dgToolBar>
   <t:dgToolBar operationCode="edit" title="common.edit" icon="icon-edit" url="bopGController.do?goUpdate" width="1036"  funname="update"></t:dgToolBar>
   <t:dgToolBar operationCode="detail" title="common.view"  icon="icon-search" url="bopGController.do?goUpdate" width="1036"  funname="detail"></t:dgToolBar>
   ...
   <t:dgToolBar  operationCode="batchApprove" title="r.batch.approve" icon="icon-ok" url="bopController.do?doBatchApprove&type=G" funname="approveALLSelect"></t:dgToolBar>
    <t:dgToolBar  operationCode="batchDel" title="common.batch.delete"  icon="icon-remove"  url="bopGController.do?doBatchDel" funname="deleteALLSelect"></t:dgToolBar>
  </t:datagrid>
  </div>
 </div>
 <script src = "webpage/safe/bop/bopGList.js"></script>		
 <script type="text/javascript">
 $(document).ready(function(){
 		//给时间控件加上样式
 			$("#bopGListtb").find("input[name='rptdate_begin']").attr("class","Wdate").click(function(){WdatePicker({dateFmt:'yyyy-MM-dd'});});
 			$("#bopGListtb").find("input[name='rptdate_end']").attr("class","Wdate").click(function(){WdatePicker({dateFmt:'yyyy-MM-dd'});});
 			$("#bopGListtb").find("input[name='importdate_begin']").attr("class","Wdate").click(function(){WdatePicker({dateFmt:'yyyy-MM-dd'});});
 			$("#bopGListtb").find("input[name='importdate_end']").attr("class","Wdate").click(function(){WdatePicker({dateFmt:'yyyy-MM-dd'});});
 });
 
//导入
function ImportXls() {
	openuploadwin('Excel导入', 'bopGController.do?upload', "bopGList");
}

//导出
function ExportXls() {
	JeecgExcelExport("bopGController.do?exportXls","bopGList");
}

//模板下载
function ExportXlsByT() {
	JeecgExcelExport("bopGController.do?exportXlsByT","bopGList");
}
function approveCancel(id) {
	createwindow('审核重置', 'bopController.do?doCancelAuthor&id=' + id+'&type=G');
}
 </script>