...
<div class="easyui-layout" fit="true">
  ...
  <t:datagrid name="bopQList" checkbox="true" fitColumns="true" title="境内汇款申请书-管理信息" actionUrl="bopQController.do?datagrid" 
   ...
   <t:dgCol title="境内收入类型"  field="payattr"    queryMode="group" dictionary="payattr" ></t:dgCol>
   <t:dgCol title="付款性质"  field="paytype"  hidden="true"  queryMode="group" dictionary="paytype" ></t:dgCol>
   ...
   <t:dgDelOpt operationCode="del" title="common.delete" exp="isvalidation#eq#0" url="bopQController.do?doDel&id={id}" />
   <t:dgToolBar operationCode="add" title="common.add" icon="icon-add" url="bopQController.do?goAdd" funname="add"></t:dgToolBar>
   <t:dgToolBar operationCode="edit" title="common.edit" icon="icon-edit" url="bopQController.do?goUpdate" width="1036"  funname="update"></t:dgToolBar>
   <t:dgToolBar operationCode="detail" title="common.view"  icon="icon-search" url="bopQController.do?goUpdate" funname="detail"></t:dgToolBar>
   ...
   <t:dgToolBar  operationCode="batchApprove" title="r.batch.approve" icon="icon-ok" url="bopController.do?doBatchApprove&type=Q" funname="approveALLSelect"></t:dgToolBar>
    <t:dgToolBar  operationCode="batchDel" title="common.batch.delete"  icon="icon-remove"  url="bopQController.do?doBatchDel" funname="deleteALLSelect"></t:dgToolBar>
  </t:datagrid>
  </div>
 </div>
 <script src = "webpage/safe/bop/bopQList.js"></script>		
 <script type="text/javascript">
 $(document).ready(function(){
 		//给时间控件加上样式
 			$("#bopQListtb").find("input[name='rptdate_begin']").attr("class","Wdate").click(function(){WdatePicker({dateFmt:'yyyy-MM-dd'});});
 			$("#bopQListtb").find("input[name='rptdate_end']").attr("class","Wdate").click(function(){WdatePicker({dateFmt:'yyyy-MM-dd'});});
 			$("#bopQListtb").find("input[name='importdate_begin']").attr("class","Wdate").click(function(){WdatePicker({dateFmt:'yyyy-MM-dd'});});
 			$("#bopQListtb").find("input[name='importdate_end']").attr("class","Wdate").click(function(){WdatePicker({dateFmt:'yyyy-MM-dd'});});
 });
 
//导入
function ImportXls() {
	openuploadwin('Excel导入', 'bopQController.do?upload', "bopQList");
}

//导出
function ExportXls() {
	JeecgExcelExport("bopQController.do?exportXls","bopQList");
}

//模板下载
function ExportXlsByT() {
	JeecgExcelExport("bopQController.do?exportXlsByT","bopQList");
}function approveCancel(id) {
	createwindow('审核重置', 'bopController.do?doCancelAuthor&id=' + id+'&type=Q');
}
 </script>