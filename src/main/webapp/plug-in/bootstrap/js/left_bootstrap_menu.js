...
function InitLeftMenu() {
	...
	// node.target);
		// node.state = node.state === 'closed' ? 'open' : 'closed';
			var isleaf= $('.easyui-tree').tree('isLeaf', node.target);
			if(!isleaf){
				openThisNoed(node);
			}else{
				addNodeToTab(node);
...
function tabCloseEven() {
	...
	$('#mm-tabupdate').click(function() {
		...
		if('undefined' == typeof(url)&&index==0){
			$("#home").load('loginController.do?home'); 
		}else{
...
$.parser.onComplete = function() {/* 页面所有easyui组件渲染成功后，隐藏等待信息 */
	if ('undefined' == typeof(document.body.style.maxHeight)) {/* 解决IE6的PNG背景不透明BUG */
	}
