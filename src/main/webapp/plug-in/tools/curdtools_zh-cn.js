...
var frameEl = window.frameElement;// 返回嵌入当前window对象的元素(比如 <iframe> 或者
									// <object>),如果当前window对象已经是顶层窗口,则返回null.
if (null != frameEl) {
	// https://developer.mozilla.org/zh-CN/docs/Web/API/Window/frameElement
	var windowapi = window.frameElement.api;
 ...
 * @param index
 */
function doSubmitAlertTip(url, name, data) {
	gridname = name;
	// --author：JueYue
 ...
 * @param index
 */
function doSubmitNunTable(url, name, data) {
	gridname = name;
	// --author：JueYue
	// ---------date：20140227---------for：把URL转换成POST参数防止URL参数超出范围的问题
	var paramsData = data;
...
function approveALLSelect(title, url, gname) {
	...
	var rows = $("#" + gname).datagrid('getSelections');
	if (rows.length > 0) {
		$.dialog.confirm('你确定审核通过这些数据吗?', function(r) {
			if (r) {
				for (var i = 0; i < rows.length; i++) {
					ids.push(rows[i].id);
				}
