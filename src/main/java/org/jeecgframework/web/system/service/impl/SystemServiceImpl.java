...
public class SystemServiceImpl extends CommonServiceImpl implements SystemService {
	...
	public Set<String> getOperationCodesByUserIdAndFunctionId(String userId, String functionId) {
		...
		StringBuffer allelm=new StringBuffer();
		int c=0;
		for (TSRoleUser ru : rUsers) {
			...
			if (null != rFunctions && rFunctions.size() > 0) {
				...
				if (null != tsRoleFunction.getOperation()) {
					c++;
					allelm.append(tsRoleFunction.getOperation());
//					String[] operationArry = s.split(",");
//					for (int i = 0; i < operationArry.length; i++) {
//						operationCodes.add(operationArry[i]);
//					}
				}
		...
		if(allelm.toString().length()>0){
			String[] s=allelm.toString().split(",");
			int count=0;
			int index=-1;
			for (String e : s) {
				count=getCounts(e,allelm.toString(),index, count);
				if(count==c){//最大限制
					TSOperation operation = new TSOperation();
			        operation.setId(e);
			        operationCodes.add(e);
				}
				index=-1;
		    	count=0;
			}
	...
	private int getCounts(String e, String allelm,int index,int count) {
		
		index=allelm.indexOf(e,index);
		if(index>=0){
			count++;
		 count=	getCounts(e,allelm.substring(index+e.length()),index,count);
	}
	return count;
	...
	public String findBrcaByShortcnname(String shortcnname ){
		CriteriaQuery cq = new CriteriaQuery(BankinfoEntity.class);
		cq.eq("shortcnname", shortcnname);
		cq.add();
	...
	public String findShortcnnameBybrca(String brca ){
		CriteriaQuery cq = new CriteriaQuery(BankinfoEntity.class);
		cq.eq("brca", brca);
		cq.add();
		BankinfoEntity bk = getUniqueObjectByCriteriaQuery(cq);
		if (null != bk) {
			return bk.getShortcnname();
		}
		return "";
	}
	/**
