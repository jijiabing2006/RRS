...
public class UserController extends BaseController {
	 ...
	 * @return
	 */
	@RequestMapping(params = "changepasswordforexp")
	public String changepasswordforexp(HttpServletRequest request) {
...
//		System.out.println(username);
	...
	public AjaxJson savenewpwd(HttpServletRequest request) {
		...
		if (!pString.equals(user.getPassword())) {
			...
			user.setExpiredate(DateUtils.addDate(new Date(), 45));		// 修改密码，更新过期时间为45天后。
	...
	public AjaxJson savenewpwdforuser(HttpServletRequest req) {
		...
		if (StringUtil.isNotEmpty(id)) {
			...
			users.setExpiredate(DateUtils.addDate(new Date(), 45));		// 重置密码，更新过期时间为45天后。
...
//				}
				user.setExpiredate(new Date());		// 新增用户时，把密码过期日期设置成当日，强制用户第一次登陆，修改密码
