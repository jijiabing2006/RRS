...
public class ListtoMenu {
	...
	public static String getEasyuiMenu(List<TSFunction> pFunctions,
		...
		for (TSFunction pFunction : pFunctions) {
			menuString.append("<div  title=\"" + getMutiLang(pFunction.getFunctionName())
					+ "\" iconCls=\"" + getIconClass(pFunction)
					+ "\">");
			/*int submenusize = pFunction.getSubFunctionSize();
			if (submenusize == 0) {
	...
	private static String getIconClass(TSFunction function) {
		return TSIcon.allTSIcons.get(function.getTSIcon().getId())!=null?TSIcon.allTSIcons.get(function.getTSIcon().getId()).getIconClas():"default";
	}
		...
		for (TSFunction function : list) {
			if (function.getTSFunction().getId().equals(parent.getId())){
				if(!function.hasSubFunction(map)){
					...
					menuString.append("<li state=\"closed\" iconCls=\"tree-" + getIconClass(function)+"\" ><span>"+ getMutiLang(function.getFunctionName()) +"</span>");
					menuString.append("<ul >");
