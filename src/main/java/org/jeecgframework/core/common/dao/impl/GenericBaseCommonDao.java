...
public abstract class GenericBaseCommonDao<T, PK extends Serializable>
	...
	private <T> void getProperty(Class entityName) {
		...
		for (int i = 0; i < str.length; i++) {
			...
			org.jeecgframework.core.util.LogUtil.info(property + "---&gt;"
					+ type);
		}
	}
	...
	public <T> List<T> getDistinctFieldValueByWhere(Class<T> entityClass,String fieldname,
			String whereql, Object[] queryParams){
		StringBuffer sf_QL = new StringBuffer("SELECT distinct(").append(fieldname).append(") FROM ")
				.append(sqlBuilder.getEntityName(entityClass)).append(
						" o ");
//	 List t=	(List<T>) this.jdbcTemplate.queryForList("select distinct(acod) from t_tran_detail");
		if (whereql != null && whereql.trim().length() != 0) {
			sf_QL.append(" WHERE ").append(whereql);
		}
		Query query = getSession().createQuery(sf_QL.toString());
	...
	public <T> T getMaxFieldValueByWhere(Class<T> entityClass,String fieldname,
			String whereql, Object[] queryParams){
		StringBuffer sf_QL = new StringBuffer("SELECT max(").append(fieldname).append(") FROM ")
				.append(sqlBuilder.getEntityName(entityClass)).append(
						" o ");
//	 List t=	(List<T>) this.jdbcTemplate.queryForList("select distinct(acod) from t_tran_detail");
		if (whereql != null && whereql.trim().length() != 0) {
	...
	public Long getCountForJdbcParam(String sql, Object[] objs) {
		return this.jdbcTemplate.queryForObject(sql, objs,
				java.lang.Long.class);
	}
	...
	public Object executeSqlReturnKey(final String sql,
	 ...
	 * @return
	 */
	public <T> List<T> findHql(Class<T> entityClass,String wheresql, Object... param) {
		String hql=" FROM "
				+ sqlBuilder.getEntityName(entityClass) + " o "
	...
	public <T > List<Object[]> queryFieldValues(
