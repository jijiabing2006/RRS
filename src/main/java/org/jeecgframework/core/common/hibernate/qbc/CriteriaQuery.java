...
public class CriteriaQuery implements Serializable {
	...
	private boolean menualsearch = false;
	private String field="";//查询需要显示的字段
	private Class entityClass;//POJO
	private List results;// 结果集
	private List<Integer> ridCriterion=new ArrayList<Integer>();// 需要去除的查询条件（因为已经在进行过‘or’，‘and’拼接）
	private int total;
	...
	public void add() {
		for (int i = 0; i < getCriterionList().size(); i++) {
			if(!ridCriterion.contains(i)){ //ridCritertion中是已经进行过‘and’或者‘or’操作的查询条件
	...
	public Criterion and(CriteriaQuery query, int source, int dest) {
		ridCriterion.add(source);
		ridCriterion.add(dest);
		Criterion c=	Restrictions.and(query.getCriterionList().getParas(source),
				query.getCriterionList().getParas(dest));
		criterionList.addPara(c);
		return c;
	}
	...
	public Criterion and(Criterion c, CriteriaQuery query, int source) {
		ridCriterion.add(source);
		Criterion nc=Restrictions.and(c, query.getCriterionList().getParas(source));
		criterionList.addPara(nc);
		return nc;
	}
	...
	public Criterion or(CriteriaQuery query, int source, int dest) {
		ridCriterion.add(source);
		ridCriterion.add(dest);
		Criterion c=Restrictions.or(query.getCriterionList().getParas(source), query
	...
	public Criterion or(Criterion c, CriteriaQuery query, int source) {
		ridCriterion.add(source);
		Criterion nc= Restrictions.or(c, query.getCriterionList().getParas(source));
		criterionList.addPara(nc);
		return nc;
	}
	 ...
	 * @param keyvalue2
	 */
	public void notlike(String keyname, Object keyvalue) {
		if (keyvalue != null && keyvalue != "") {
			//criterionList.addPara(Restrictions.like(keyname, "%" + keyvalue+ "%"));
			criterionList.addPara(Restrictions.not(Restrictions.like(keyname, keyvalue)));
			if (flag) {
				this.put(keyname, keyvalue);
			}
			flag = true;
	 ...
	 * @param keyname
	 */
	public void max(String keyname) {
		detachedCriteria.setProjection(Projections.max(keyname));
	}
	/**
	 * 设置Min查询条件
	 * 
	 * @param keyname
	 */
	public void min(String keyname) {
		detachedCriteria.setProjection(Projections.min(keyname));
	}
	/**
	 * 设置distinct查询条件
	 * 
	 * @param keyname
	 */
	public void distinct(Projection keyname) {
		detachedCriteria.setProjection(Projections.distinct(keyname));
	}
	/**
	 * 保存查询条件
	 * 
	...
	public boolean isMenualsearch() {
		return menualsearch;
	}

	public void setMenualsearch(boolean menualsearch) {
		this.menualsearch = menualsearch;
	}
