...
/**body > div.easyui-layout.layout > div.panel.layout-panel.layout-panel-center > div > div > div.datagrid-wrap.panel-body > div.datagrid-view > div.datagrid-view2 > div.datagrid-header > div > table > tbody > tr > td:nth-child(17) > div
 * 
 * 类描述：DATAGRID标签处理类
 * 
 @author 张代浩
...
public class DataGridTag extends TagSupport {
	...
	public void setColumn(String title, String field, Integer width, String rowspan, 
			...
			String style,String downloadName,boolean isAuto,String extendParams,String exp) {
...
//			}
	...
	public StringBuffer end() {
		...
		if (columnList.size() > 0) {
			...
			sb.append("$(\'#" + name + "\')." + grid + "({url:'" + actionUrl + "&menualsearch&field=" + searchFields + "',pageNumber:1});" + "}");
			...
			if("group".equals(getQueryMode())){
				for (DataGridColumn col : columnList) {
					if (col.isQuery()) {
						...
						sb.append("<span style=\"vertical-align:middle;display:-moz-inline-box;display:inline-block;width: 120px;text-align:left;text-overflow:ellipsis;-o-text-overflow:ellipsis; overflow: hidden;white-space:nowrap; \" title=\""+col.getTitle()+"\">"+col.getTitle()+"：</span>");
						if("single".equals(col.getQueryMode())){
							if(!StringUtil.isEmpty(col.getReplace())){
								...
								sb.append("<input class=\"input_text\" onkeypress=\"EnterPress(event)\" onkeydown=\"EnterPress()\"  type=\"text\" name=\""+col.getField().replaceAll("_","\\.")+"\"  "+extendAttribute(col.getExtend())+" style=\"width: 100px\" />");
							}
						}else if("group".equals(col.getQueryMode())){
							sb.append("<input class=\"input_text\" type=\"text\" name=\""+col.getField()+"_begin\"  style=\"width: 94px\" "+extendAttribute(col.getExtend())+"/>");
							sb.append("<span style=\"display:-moz-inline-box;display:inline-block;width: 8px;text-align:right;\">~</span>");
							sb.append("<input class=\"input_text\" type=\"text\" name=\""+col.getField()+"_end\"  style=\"width: 94px\" "+extendAttribute(col.getExtend())+"/>");
						}
						sb.append("</span>");
					}
				}
	...
	protected void getOptUrl(StringBuffer sb) {
		...
		for (DataGridUrl dataGridUrl : list) {
			...
			if (StringUtil.isNotEmpty(exp)) {// 判断显示表达式 结尾    （与开头方法要匹配）
	...
	private void parseExp(StringBuffer sb, String exp) {
			...
			if (columnValueList.size() > 0 && !column.getField().equals("opt")) {
				...
				for (ColumnValue columnValue : columnValueList) {
					if (columnValue.getName().equals(column.getField())) {
						...
						sb.append("}");
						sb.append("else{");
						if (StringUtil.isNotEmpty(exp)) {// 判断显示表达式  开头
							parseExp(sb, exp);
						}
						for (int j = 0; j < value.length; j++) {
						...
						if (StringUtil.isNotEmpty(exp)) {// 判断显示表达式 结尾    （与开头方法要匹配）
							for (int j = 0; j < exp.split("&&").length; j++) {
								sb.append("}");
							}
	...
	private String getAutoSpan(String filed,String extend){
        ...
        .append("</script>")
        .append("<input class=\"input_text\" type=\"text\" id=\""+getEntityName()+"_"+id+"\" name=\""+filed+"\" datatype=\"*\" "+extend+ StringUtil.replace(" nullmsg=\"\" errormsg=\"{0}\"/>", "{0}", MutiLangUtil.getMutiLangInstance().getLang("input.error")));
		return nsb.toString();
	}
	/**
	 * 获取实体类名称,没有这根据规则设置
		...
		if(hasQueryColum(columnList)){
			...
			if("group".equals(getQueryMode())){
				for (DataGridColumn col : columnList) {
					if (col.isQuery()) {
						sb.append("<span style=\"display:-moz-inline-box;display:inline-block;\">");
						sb.append("<span style=\"vertical-align:middle;display:-moz-inline-box;display:inline-block;width: 120px;text-align:right;text-overflow:ellipsis;-o-text-overflow:ellipsis; overflow: hidden;white-space:nowrap; \" title=\""+col.getTitle()+"\">"+col.getTitle()+"：</span>");
						if("single".equals(col.getQueryMode())){
							if(!StringUtil.isEmpty(col.getReplace())){
								sb.append("<select name=\""+col.getField().replaceAll("_","\\.")+"\" WIDTH=\"100\" style=\"width: 104px\"> ");
								sb.append(StringUtil.replaceAll("<option value =\"\" >{0}</option>", "{0}", MutiLangUtil.getMutiLangInstance().getLang("common.please.select")));
								...
								sb.append("<input class=\"input_text\" onkeypress=\"EnterPress(event)\" onkeydown=\"EnterPress()\"  type=\"text\" name=\""+col.getField().replaceAll("_","\\.")+"\"  "+extendAttribute(col.getExtend())+"  />");
							}
						}else if("group".equals(col.getQueryMode())){
							sb.append("<input class=\"input_text\" type=\"text\" name=\""+col.getField()+"_begin\"   "+extendAttribute(col.getExtend())+"/>");
							sb.append("<span style=\"display:-moz-inline-box;display:inline-block;width: 8px;text-align:right;\">~</span>");
							sb.append("<input class=\"input_text\" type=\"text\" name=\""+col.getField()+"_end\"   "+extendAttribute(col.getExtend())+"/>");
