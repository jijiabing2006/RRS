...
public class ComboBoxTag extends TagSupport {
	...
	protected String defaultvalue;//下拉框默认值（初始值 ）
	...
	public StringBuffer end() {
		...
		sb.append("<script type=\"text/javascript\">"
				...
				+"onChange:function(){"
				+"var val = $(\'#"+name+"\').combobox(\'getValue\');"
				+"$(\'#"+name+"hidden\').val(val);"
				+"},"
				...
				+"});"
				+"</script>");
		sb.append("<input type=\"hidden\" name=\""+field+"\" id=\""+name+"hidden\" > "
				+"<input class=\"easyui-combobox\" "
				+"  data-options=\" multiple:false\" value=\""+defaultvalue+"\" name=\""+name+"name\" id=\""+name+"\" >");
		return sb;
	}
